...
public class EvenlySpacedLayout extends ViewGroup
{
	...
	{
		super(context, attrs);
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.GalleryLayout, 0, 0);
		mHorizontal = (0 == a.getInt(R.styleable.GalleryLayout_orientation, 0));
		mKeepEndSpace = a.getBoolean(R.styleable.GalleryLayout_keepEndSpace, true);
		a.recycle();
	}
	...
	void layoutHorizontal(boolean changed, int l, int t, int r, int b)
	{
		...
		int spacing = (r - l - usedWidth) / (mKeepEndSpace ? (usedChildren + 1) : (usedChildren - 1));
	...
	void layoutVertical(boolean changed, int l, int t, int r, int b)
	{
		...
		int spacing = (b - t - usedHeight) / (mKeepEndSpace ? (usedChildren + 1) : (usedChildren - 1));
