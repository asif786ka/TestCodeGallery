...
public class ImageList extends BaseImageList implements IImageList
{
	...
	static final String		TAG						= "ImageList";

	static final String[]	ACCEPTABLE_IMAGE_TYPES	= new String[] {
	...
	public HashMap<String, String> getBucketIds()
	{
		...
		Cursor cursor = Media.query(mContentResolver, uri, new String[] {
				Media.BUCKET_DISPLAY_NAME, Media.BUCKET_ID }, whereClause(), whereClauseArgs(), null);
		try
		{
	...
	{
		Cursor c = Media.query(mContentResolver, mBaseUri, IMAGE_PROJECTION, whereClause(), whereClauseArgs(),
				sortOrder());
		return c;
	}

	static final String[]	IMAGE_PROJECTION		= new String[] {
			Media._ID, Media.DATA, Media.DATE_TAKEN, Media.MINI_THUMB_MAGIC, Media.ORIENTATION, Media.TITLE,
			Media.MIME_TYPE, Media.DATE_MODIFIED	};

	static final int		INDEX_ID				= 0;
	static final int		INDEX_DATA_PATH			= 1;
	static final int		INDEX_DATE_TAKEN		= 2;
	static final int		INDEX_MINI_THUMB_MAGIC	= 3;
	static final int		INDEX_ORIENTATION		= 4;
	static final int		INDEX_TITLE				= 5;
	static final int		INDEX_MIME_TYPE			= 6;
	...
	protected BaseImage loadImageFromCursor(Cursor cursor)
	{
		...
		long lastModified = cursor.getLong(INDEX_DATE_MODIFIED) * 1000;
		...
		return new Image(this, mContentResolver, id, cursor.getPosition(), contentUri(id), dataPath, mimeType,
				dateTaken, lastModified, title, orientation);
	}
}
